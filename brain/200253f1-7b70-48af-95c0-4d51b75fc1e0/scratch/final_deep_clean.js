...
// 2. Global Deep Cleaning
console.log("Performing global deep cleaning...");
...
content = content
    ...
    .replace(/<body.*?>/gi, '')
    .replace(/<\/body>/gi, '')
    // Remove meta and title tags (usually at the start of messy imports)
    .replace(/<meta\s+[^>]*>/gi, '')
    .replace(/<title>.*?<\/title>/gi, '')
    // Replace blogger images with placeholders (as requested)
...
// 3. Final Verification of Mojibake Patterns
const mojibakePatterns = [/Ã¡/g, /Ã©/g, /Ã­/g, /Ã³/g, /Ãº/g, /Ã±/g, /Ã\?/g];
let mojibakeCount = 0;
...
if (mojibakeCount > 0) {
    console.log(`Found ${mojibakeCount} potential Mojibake patterns. Fixing...`);
    content = content
        .replace(/Ã¡/g, 'á')
        .replace(/Ã©/g, 'é')
        .replace(/Ã­/g, 'í')
