...
const backupPath = 'c:\\Users\\comun\\OneDrive\\Documentos\\GitHub\\Protocolo-Recep\\data.js.bak';
const dataPath = 'c:\\Users\\comun\\OneDrive\\Documentos\\GitHub\\Protocolo-Recep\\data.js';
const tempPath = path.join(__dirname, 'temp_protocols_v3.js');
...
let rawContent = getRepairedContent(backupPath);
...
protocols.forEach(p => {
    if (!p.video_url && p.content) {
        ...
        if (iframeMatch) p.video_url = iframeMatch[1];
    }
    
    if (p.content) p.content = sanitizeContent(p.content);
...
console.log("Migration completed successfully with REPAIRED encoding.");
